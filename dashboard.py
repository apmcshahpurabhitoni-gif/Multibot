...
def build_dashboard_snapshot(*,version=APP_VERSION,whats_new=WHAT_IS_NEW,accounts=(),signals=(),trades=(),scan=None,health=None,strategies=()):
    ...
    fresh_directional=[x for x in directional if str(x.get("freshness","")).upper()=="FRESH"]
    stale_directional=[x for x in directional if str(x.get("freshness","")).upper()=="STALE"]
    delivery_counts={}

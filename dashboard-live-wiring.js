...
(function(){
  ...
  const groupToggle=(kind,key,open,label,sub)=>`<button class="history-date-toggle" type="button" data-history-date="${escapeHtml(key)}" data-history-kind="${kind}" aria-expanded="${open}"><span><b>${escapeHtml(label)}</b><small>${escapeHtml(sub)}</small></span><i aria-hidden="true">⌄</i></button>`;

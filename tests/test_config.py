...
def test_forex_universe_uses_approved_pairs():assert FOREX_SYMBOLS==("EURUSD=X","GBPUSD=X","AUDUSD=X","USDJPY=X","NZDUSD=X","EURJPY=X") and all(symbol in LIVE_ASSET_MAP for symbol in FOREX_SYMBOLS)
...
def test_live_universe_contains_25_assets():assert len(LIVE_ASSETS)==25 and len(set(asset.symbol for asset in LIVE_ASSETS))==25

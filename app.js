...
function expandButton(key,label){const open=state.expanded.has(key);const card=open?"expanded":"";return `<button class="expand-button collapse-control ${open?"is-open":""}" type="button" data-expand="${escapeHtml(key)}" aria-expanded="${open}" aria-label="${open?"Collapse":"Expand"} ${escapeHtml(label)}" aria-hidden="true" tabindex="-1"></button>`;}
  ...
  return days.map((day,index)=>{
    ...
    return `<section class="calendar-date-group ${open?"is-open":""}">
      <button class="calendar-date-toggle" type="button" data-calendar-date-group="${escapeHtml(key)}" aria-expanded="${open}">
        ...
        <i aria-hidden="true" class="chev collapse-control"></i>
      </button>

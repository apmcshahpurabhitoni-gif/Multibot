...
function expandButton(key,label){const open=state.expanded.has(key);const card=open?"expanded":"";return `<button class="expand-button collapse-control ${open?"is-open":""}" type="button" data-expand="${escapeHtml(key)}" aria-expanded="${open}" aria-label="${open?"Collapse":"Expand"} ${escapeHtml(label)}"></button>`;}

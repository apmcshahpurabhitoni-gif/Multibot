...
NIFTY_SYMBOLS: Final[tuple[str, ...]] = ("^NSEI", "^NSEBANK")
GOLD_SYMBOL: Final[str] = "GC=F"
BITCOIN_SYMBOL: Final[str] = "BTC-USD"

# Approved Forex pairs recovered from the project specification/message contract.
FOREX_SYMBOLS: Final[tuple[str, ...]] = ("EURUSD=X", "GBPUSD=X")
...
def _equity(symbol: str) -> AssetConfig:
    return AssetConfig(symbol, symbol, f"{symbol}.NS", "NSE", "equity", "NSE Stocks", "INR", "4H")

LIVE_ASSETS: tuple[AssetConfig, ...] = tuple(_equity(symbol) for symbol in NSE_15_SYMBOLS) + (
    AssetConfig("^NSEI", "NIFTY 50", "^NSEI", "NSE", "index", "NSE Indices", "INR", "1H"),
    AssetConfig("^NSEBANK", "BANK NIFTY", "^NSEBANK", "NSE", "index", "NSE Indices", "INR", "1H"),
    AssetConfig("GC=F", "Gold (XAU/USD)", "GC=F", "Gold", "commodity", "Global Markets", "USD", "4H"),
    AssetConfig("BTC-USD", "Bitcoin (BTC)", "BTC-USD", "Crypto", "crypto", "Global Markets", "USD", "4H"),
    AssetConfig("EURUSD=X", "EUR/USD", "EURUSD=X", "Forex", "forex", "Global Markets", "USD", "4H"),
    AssetConfig("GBPUSD=X", "GBP/USD", "GBPUSD=X", "Forex", "forex", "Global Markets", "USD", "4H"),
)

LIVE_ASSET_MAP: Final[dict[str, AssetConfig]] = {asset.symbol: asset for asset in LIVE_ASSETS}
LIVE_SYMBOLS: Final[tuple[str, ...]] = tuple(asset.symbol for asset in LIVE_ASSETS)
...
BACKTEST_ASSETS = {
    asset.symbol: {
        "label": asset.label, "ticker": asset.yahoo_symbol, "market": asset.market,
        "asset_type": asset.asset_type, "group": asset.group, "currency": asset.currency,
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            timezone=timezone, timeframe=timeframe, freshness_hours=SIGNAL_FRESHNESS_HOURS,
            market_data_provider=provider, telegram_bot_token=os.getenv("TELEGRAM_BOT_TOKEN"),
...
def validate_configuration() -> None:
    ...
    if len(LIVE_ASSETS) != 21:
        raise ValueError(f"Live universe must contain exactly 21 assets; found {len(LIVE_ASSETS)}")
    if len(set(LIVE_SYMBOLS)) != 21:

...
class SweepV2Strategy(Strategy):
    manifest = StrategyManifest(
        id="sweep_v2", name="Sweep V2", version="2.0.1",
    ...
    def generate_signal(self, symbol, candles, *, now):
        asset = LIVE_ASSET_MAP[symbol]; timeframe = asset.sweep_timeframe
        ...
        if not (float(current.high)>float(previous.high) and float(current.low)<float(previous.low)):
            return Signal(self.manifest.name,self.manifest.version,symbol,"NO_SIGNAL",ts,timeframe,"NO_SWEEP")
        ...
        return Signal(self.manifest.name,self.manifest.version,symbol,direction,ts,timeframe,direction,entry,sl,tp,
            {"candle_start":pd.Timestamp(ts).isoformat(),"previous":{k:float(previous[k]) for k in ("open","high","low","close")},"current":{k:float(current[k]) for k in ("open","high","low","close")}})

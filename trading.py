...
def validate_risk_configuration():
    if ACCOUNT_SIZE_INR != 100_000:
        raise TradingRuleError("Account size must be ₹100,000")
    if RISK_PER_TRADE_INR != 2_000:
        raise TradingRuleError("Risk per trade must be ₹2,000")
    if ACCOUNT_TRADE_LIMITS != {"macro": 20, "nifty": 5, "ny_session": 3, "sweep_4h": 3}:
        raise TradingRuleError("Per-account trade limits are incorrect")
    if LEVERAGE != 1.0:
...
def can_open_trade(account):
    ...
    return (
        ...
        and account.planned_risk_used + RISK_PER_TRADE_INR <= account.max_daily_planned_risk
    )
...
def register_trade(account, *, planned_risk=RISK_PER_TRADE_INR):
    ...
    if planned_risk <= 0 or planned_risk > RISK_PER_TRADE_INR + 1e-9:
        raise TradingRuleError("Trade planned risk exceeds ₹2,000")
    return AccountState(
...
def quantity_for_risk(entry, stop_loss, *, risk_inr=RISK_PER_TRADE_INR, fx_rate=1.0):
    ...
    if risk <= 0 or risk > RISK_PER_TRADE_INR:
        raise TradingRuleError("Risk must be positive and no greater than ₹2,000")
    if rate <= 0:
